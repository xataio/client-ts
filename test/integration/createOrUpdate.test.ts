...
describe('record create or update', () => {
  ...
  test('create or update many without getting rate limited', async () => {
    ...
    const result = await xata.db.users.createOrUpdate(
      Array.from({ length: 500 }).map((_, i) => ({
        id: `user-${i}`,
        full_name: `user-${i}`,
        email: `user-${i}@myusers.com`,
        address: { street: '123 Main St' },
        settings: { dark: true, labels: ['foo', 'bar'] },
        team
      }))
    );

    expect(result).toHaveLength(500);
    expect(result.every((item) => item.id.startsWith('user-'))).toBeTruthy();
  });
});

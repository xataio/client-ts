...
import { File, Suite, TestContext, afterAll, afterEach, beforeAll, beforeEach, describe, expect, test } from 'vitest';
import { Schemas } from '../../packages/client/src';
...
describe('External Auth.js tests', async () => {
  const { baseClient: client, hooks } = await setUpTestEnvironment('next-auth', { schema });
  ...
  const options: TestOptions = {
    ...
    db: {
      ...
      async account({ provider, providerAccountId }) {
        const data = await client.db.nextauth_accounts.filter({ provider, providerAccountId }).getFirst();
        if (!data) return null;
        return data;
      },
      async session(sessionToken) {
        const data = await client.db.nextauth_sessions.filter({ sessionToken }).getFirst();
        if (!data) return null;
        return data;
      },
      async verificationToken(where) {
        const data = await client.db.nextauth_verificationTokens.filter(where).getFirst();
        if (!data) return null;
        return data;
      }
    }
  ...
  afterAll(async (suite) => {
    ...
    options.hooks?.afterAll?.(suite);
  });

  beforeEach(async (ctx) => {
    await options.hooks?.beforeEach(ctx);
  });

  afterEach(async (ctx) => {
    await options.hooks?.afterEach(ctx);
  });

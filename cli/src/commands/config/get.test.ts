...
describe('get config', () => {
  test('fails if there is not project configuration', async () => {
    ...
    const command = new GetConfig(['foo'], config as Config);
  ...
  test('fails if the key points to an object', async () => {
    const config = await Config.load();
    const command = new GetConfig(['codegen'], config as Config);
  ...
  test('returns a value', async () => {
    const config = await Config.load();
    const command = new GetConfig(['codegen.output'], config as Config);

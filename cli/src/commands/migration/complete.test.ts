...
export const fetchRunningMigrationWithSuccessfulComplete = (url: string, request: any) => {
  if (url === `${baseUrl}/migrations/status` && request.method === 'GET') {
    return {
      ok: true,
      json: async () => {
  ...
  if (url === `${baseUrl}/migrations/complete` && request.method === 'POST') {
    return {
      ok: true,
      json: async () => {
...
describe('migration complete', () => {
  test('correctly detects if there is no migration to complete', async () => {
    ...
    expect(error).toHaveBeenCalledWith(`No active migration found, there is nothing to complete.`);
  });
